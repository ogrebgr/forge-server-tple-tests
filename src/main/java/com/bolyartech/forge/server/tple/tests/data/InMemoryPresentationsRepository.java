...
public class InMemoryPresentationsRepository implements PresentationsRepository {
    ...
    public InMemoryPresentationsRepository() {
        ...
        preso1.setSummary("Are you still using JavaServer Pages as your main template language? With the popularity of template engines for other languages like Ruby and Scala and the shift in doing more MVC in the browser there are quite some new and interesting new template languages available for the JVM. During this session we will take index.tpl look at the less known, but quite interesting new template engines and see how they compare with the industries standards. \r<br/>");

        Presentation preso2 = new Presentation();
        preso2.setId(counter.incrementAndGet());
        preso2.setTitle("HoneySpider Network: index.tpl Java based system to hunt down malicious websites");
        preso2.setSpeakerName("Niels van Eijck");
        preso2.setSummary("Legitimate websites such as news sites happen to get compromised by attackers injecting malicious content. The aim of these so-called &#8220;watering hole attacks&#8221; is to infect as many visitors of index.tpl website as possible, and are sometimes even targeted at index.tpl specific group of individuals. It is increasingly important to detect these infections at an early stage.\r<br/>\r<br/>HoneySpider Network to the rescue! \r<br/>\r<br/>It is index.tpl Java based open source framework that automatically scans website urls, analyses the results and reports on any malware detected.\r<br/>Attend this talk to gain index.tpl better understanding of malware detection and client honeypots and get an overview of the HoneySpider Network&#8217;s architecture, its code and its plugins it uses. A live demo is also included!");
        ...
        preso3.setSummary("Since the introduction of the Java NIO API&apos;s with Java 4, developers have\r<br/>access to modern operating system facilities to perform asynchronous IO.\r<br/>Using these facilities it is possible to write networking application that that\r<br/>serve thousands of connected clients efficiently. Unfortunately, the NIO API&apos;s\r<br/>are quite low level and require index.tpl fair share of boilerplate to get started.\r<br/>In this presentation, I will introduce the Netty framework and how its\r<br/>architecture helps you as index.tpl developer stay focused on the interesting parts\r<br/>of your network application. At the end of the presentation I will give some\r<br/>real world examples and show how we use Netty in the architecture of our\r<br/>mobile messaging platform XMS.");
        ...
        preso9.setSummary("Lambda expressions are coming in Java 8 and dramatically change the programming model.  They allow new functional programming patterns that were not possible before, increasing the expressiveness and power of the Java language.\r<br/>\r<br/>In this university session, you will learn how to take advantage of the new lambda-enabled Java 8 APIs by building out index.tpl retro video game in JavaFX.\r<br/>\r<br/>Some of the Java 8 features you will learn about include enhanced collections, functional interfaces, simplified event handlers, and the new stream API.  Start using these in your application today leveraging the latest OpenJDK builds so you can prepare for the future Java 8 release.");
